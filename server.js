/**
 * YT7 Multi-Uploader v12.2 — Bugfixed
...
async function addLog(entry) {
  // FIX: race condition — lpush+ltrim দিয়ে atomic insert
  // আগের get→modify→set pattern-এ concurrent upload-এ log entry হারাতো।
  // lpush atomic, ltrim atomic — দুটো call-এর মাঝে race হলেও data হারায় না।
  const newEntry = { ...entry, ts: new Date().toISOString() };
  await redis.lpush('upload_log_v2', JSON.stringify(newEntry));
  await redis.ltrim('upload_log_v2', 0, 299); // max 300 entries রাখো
}
...
async function ensureTikTokToken(cfg, ch) {
  ...
  // FIX: race condition — fresh config load করে শুধু এই channel-এর token আপডেট করো
...
async function queryTikTokCreatorInfo(token) {
  ...
  const d = await r.json();
  // FIX: HTTP error বা error object যেকোনোটাই catch করো
  if (!r.ok) throw new Error('TikTok creator_info HTTP ' + r.status + ': ' + JSON.stringify(d));
  if (d.error && (!d.error.code || d.error.code !== 'ok')) throw new Error('TikTok creator_info: ' + (d.error.message || d.error.code || JSON.stringify(d.error)));
  return d.data || {};
}
async function uploadToTikTok(cfg, ch, fileInfo, title, token) {
  ...
  if (ch.tk_use_pull_from_url !== false && fileInfo.publicUrl) {
    ...
    const d = await r.json();
    if (d.error?.code && d.error.code !== 'ok') throw new Error('TikTok init (PULL): ' + JSON.stringify(d.error));
  ...
  if (initD.error?.code && initD.error.code !== 'ok') throw new Error('TikTok init (FILE): ' + JSON.stringify(initD.error));
...
// FIX: IG container polling — async, 10min timeout, non-blocking
async function uploadToInstagram(ch, fileInfo, caption) {
  ...
  // FIX: 10 min timeout (120 tries × 5s), বেশি সময় দেওয়া হলো
  const MAX_TRIES = 120;
...
async function processChannel(platform, ch) {
  ...
  // FIX: state আপলোডের পরেই save — আগে save করলে failure এ file skip হয়ে যায়
  ...
  // FIX: আপলোড সফল হলেই state save — failure তে file রোটেশনে থাকে
...
// =============== JOBS ===============
// FIX: job status এখন 'running' | 'done' | 'error' | 'partial' — সঠিকভাবে set হবে
const jobs = {};
const jobTimers = {};
function newJob() {
  const id = `job_${Date.now()}_${Math.random().toString(36).slice(2,7)}`;
  jobs[id] = { status:'running', log:[], started: new Date().toISOString(), total:0, ok:0, failed:0 };
  // FIX: 6 ঘণ্টা max — 30min এ delete হলে long upload এ "Job not found" হত
  jobTimers[id] = setTimeout(() => { delete jobs[id]; delete jobTimers[id]; }, 6*60*60*1000);
  return id;
}
function finishJob(id) {
  // Job done হলে 10min পর clean করো — frontend এর poll করার সময় পাবে
...
async function runUploadJob(targets) {
  const jobId = newJob();
  // FIX: total আগে set না করে actual runnable count পরে set করো
  // আগে targets.length দিলে skip/already_running গুলোও count হতো — frontend বিভ্রান্ত হত
  const cfg = await loadConfig();
  const promises = targets.map(async ({ platform, chId }) => {
    ...
    try {
      ...
      jobLog(jobId, `❌ ${key} failed: ${e.message}`);
      if (jobs[jobId]) jobs[jobId].failed++;
      await addLog({ platform, chId, channel: ch?.name || key, status:'error', error: e.message });
      await tg(`Platform: *${platform}*\nChannel: *${ch?.name || key}*\nError: \`${e.message.slice(0,200)}\``, true);
      return { platform, chId, status:'error', error: e.message };
    } finally { delete channelJobs[key]; }
  });
  // FIX: সব job শেষে সঠিক status set করো
...
// FIX: প্রতি 30s Redis call না করে 2min cache রাখো
let schedulerConfigCache = null;
let schedulerConfigCacheAt = 0;
const SCHEDULER_CACHE_TTL = 2 * 60 * 1000; // 2 minutes
...
// FIX: job status এখন 'done' | 'error' | 'partial' সঠিকভাবে return করে
...
// FIX: /api/status — সব channel state একসাথে batch load করো (N Redis calls → mget)
app.get('/api/status', async (req, res) => {
  ...
  // সব state keys collect করো
  const allKeys = [];
  for (const p of PLATFORMS) for (const ch of (cfg.channels[p] || [])) allKeys.push(`state:${p}:${ch.id}`);

  // FIX: একটাই mget call — N Redis calls এর বদলে ১টা
  let allStates = [];
  if (allKeys.length > 0) {
    try { allStates = await redis.mget(...allKeys); } catch (e) {
...
app.get('/api/time', async (req, res) => {
  ...
  const cfg = await loadConfigCached(); // FIX: cached version ব্যবহার
  const fired = await loadFired();
  let nextSlots = [];
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log(`🚀 YT7 Multi-Uploader v12.2 (bugfixed) on port ${PORT}`);
    startScheduler();
  });
} else {
